...
class CustiomUICollectionViewLayout: UICollectionViewLayout {
    ...
    override func prepare() {
        ...
        guard let collectionView = collectionView,
              let sectionCount = collectionView.dataSource?.numberOfSections?(in: collectionView) else { return }
        
        
        // Reset cached information.
        cachedAttributes.removeAll()
        contentBounds = collectionView.bounds.size
        ...
        var xOffset = 0;
        var yOffset = 0;
        let cvWidth = Int(collectionView.bounds.size.width)
        for section in 0..<sectionCount {
            let itemCount = collectionView.numberOfItems(inSection: section)
            for item in 0..<itemCount {
                let attributes = UICollectionViewLayoutAttributes(forCellWith: IndexPath(item: item, section: section))
                
                attributes.frame = CGRect(x: 0, y: yOffset + 10, width: cvWidth, height: 200)
                cachedAttributes.append(attributes)
                yOffset += 210;
            }
           
        }
        contentBounds = CGSize(width: xOffset, height: yOffset)
